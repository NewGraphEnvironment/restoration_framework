...
# Load NGE canonical styles from gq package
reg <- gq_registry_read(
  system.file("registry", "reg_qgis_restoration.json", package = "gq")
)
lake_style <- gq_mapgl_style(reg$layers$lake)
stream_label_style <- gq_mapgl_style(reg$layers$stream_labels)
```
...
maplibre(
  ...
  # River labels — styled from gq registry
  add_symbol_layer(
    id = "river-labels",
