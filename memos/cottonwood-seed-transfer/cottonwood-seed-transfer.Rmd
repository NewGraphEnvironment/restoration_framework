...
# Tighter bbox: PG to Mackenzie to Smithers — just what we need
# ~122-127W, 53.5-56N
bbox_wgs <- st_bbox(c(xmin = -127, ymin = 53.5, xmax = -122, ymax = 56), crs = 4326)
  ...
  query = "
    SELECT gnis_name_1 as name,
           ST_Transform(ST_Simplify(
             ST_Intersection(geom, ST_Transform(ST_MakeEnvelope(-127, 53.5, -122, 56, 4326), 3005)),
             500), 4326) as geom
    ...
    AND ST_Intersects(geom, ST_Transform(ST_MakeEnvelope(-127, 53.5, -122, 56, 4326), 3005))
  "
)
...
streams <- st_read(
  ...
  query = "
    ...
    WHERE stream_order >= 7
      AND ST_Intersects(
        geom,
        ST_Transform(ST_MakeEnvelope(-127, 53.5, -122, 56, 4326), 3005)
      )
  "
)
```
